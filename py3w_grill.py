...
"""
    ...
    based on work by <Lawrence Glaister> and <John Thornton>  -- 
    thanks Lawrence and John!
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.WriteToAxis = Button(self, text='Write to Axis', command=self.WriteToAxis)
        self.WriteToAxis.grid(row=14, column=2, sticky=S)
...
#            return
        ...
        if self.ShapeVar.get() == 0:    # circular grid
            yholes = int(((float(self.GrillX.get())/float(self.HoleSpaceVar.get()))+3.0)/math.cos(math.pi/6))
        else:
            yholes = int(((float(self.GrillY.get())/float(self.HoleSpaceVar.get()))+3.0)/math.cos(math.pi/6))
        ...
        if self.PatternVar.get() == 0:   # Triangle hole pattern
            PercentOpenArea = 100*math.pi*float(self.DrillVar.get())*float(self.DrillVar.get())/6.0/float(self.HoleSpaceVar.get())/float(self.HoleSpaceVar.get())/math.tan(math.pi/6.0)
        ...
        if self.PatternVar.get()==0:   #Triangle
            YSpacing = math.sqrt((Spacing * Spacing) - ((Spacing/2.0) * (Spacing/2.0)))
            XOffset = Spacing/2.0
        else:
            YSpacing = Spacing
            XOffset = 0.0
